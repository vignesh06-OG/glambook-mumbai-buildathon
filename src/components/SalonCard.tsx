...
export function SalonCard({ salon }: Props) {
  ...
  return (
    <Link
      href={`/salon/${salon.id}`}
      className="group flex flex-col overflow-hidden rounded-2xl border border-rose-100 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-rose-100"
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <p className="mt-auto pt-3 text-sm text-stone-600">
          From <span className="font-semibold text-stone-900">{formatPrice(minPrice)}</span>
